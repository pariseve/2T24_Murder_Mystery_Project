...
public class OliverDay1TownDialogue : MonoBehaviour
{
    ...
    private const string CONVERSATION_TRIGGERED_KEY = "OliverDay1TownConversationTriggered";
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (!PlayerPrefs.HasKey(CONVERSATION_TRIGGERED_KEY) && BoolManager.Instance.GetBool("SheriffIanDay1Town1"))
            {
                ShowInteractText();
            }
    ...
    private void Update()
    {
        ...
        {
            if (BoolManager.Instance.GetBool("SheriffIanDay1Town1"))
            {
                StartConversation();
            }
            else
            {
                Debug.Log("Cannot start conversation: bool is false");
            }
        }
    }
